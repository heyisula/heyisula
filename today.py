...
QUERY_COUNT = {'user_getter': 0, 'follower_getter': 0, 'graph_repos_stars': 0, 'recursive_loc': 0, 'graph_commits': 0, 'loc_query': 0, 'contribution_calendar': 0}
...
def loc_query(owner_affiliation, comment_size=0, force_cache=False, cursor=None, edges=[]):
    ...
    if request.json()['data']['user']['repositories']['pageInfo']['hasNextPage']:   # If repository data has another page
        edges += [e for e in request.json()['data']['user']['repositories']['edges'] if e['node'] is not None]  # Add on to the LoC count, skipping inaccessible repos
        return loc_query(owner_affiliation, comment_size, force_cache, request.json()['data']['user']['repositories']['pageInfo']['endCursor'], edges)
    else:
        return cache_builder(edges + [e for e in request.json()['data']['user']['repositories']['edges'] if e['node'] is not None], comment_size, force_cache)
...
def stars_counter(data):
    ...
    for node in data:
        if node['node'] is None:
            continue  # inaccessible repo (e.g. an org repo the token isn't approved for) — skip it
...
def contribution_calendar(username):
    """
    Uses GitHub's GraphQL v4 API to return the past year's daily contribution calendar
    (same window shown on the GitHub profile contribution graph).
    """
    ...
    request = simple_request(contribution_calendar.__name__, query, {'login': username})
...
def compute_streaks(weeks):
    ...
    for d in days:
        if d['contributionCount'] > 0:
            ...
            current = 0
    # current streak = trailing run ending at the last day in the dataset
    trailing = 0
...
def build_heatmap_markup(weeks, total_contributions, mode):
    ...
    standard GitHub contribution graph: 14px week columns starting at x=28, 14px day
    rows starting at y=22 (Sun..Sat).
    """
    colors = DARK_HEATMAP_COLORS if mode == 'dark' else LIGHT_HEATMAP_COLORS
    ...
    for wi, week in enumerate(weeks):
        ...
        if first_day:
            ...
            if month != last_month:
                parts.append(f'<text x="{x}" y="14" class="month">{MONTH_ABBR[month-1]}</text>')
                last_month = month
        for day in week['contributionDays']:
            ...
            title = f"{count} {noun} on {day['date']}" if count else f"No contributions on {day['date']}"
            parts.append(
                f'<rect x="{x}" y="{y}" width="11" height="11" rx="2.5" fill="{colors[lvl]}" '
                f'opacity="0" transform="translate(0,-6)"><title>{title}</title>'
                ...
                f'begin="{delay:.3f}s" dur="0.35s" fill="freeze" calcMode="spline" keySplines="0.2 0 0.2 1" /></rect>'
            )
        delay += 0.015

    parts.append('<text x="0" y="45" class="dow">Mon</text>')
    parts.append('<text x="0" y="73" class="dow">Wed</text>')
    parts.append('<text x="0" y="101" class="dow">Fri</text>')

    parts.append('<text x="28" y="144" class="legend-label">Less</text>')
...
def update_heatmap_svg(filename, weeks, total_contributions, mode):
    ...
    group = root.find(".//svg:g[@class='heatmap']", ns)
    if group is None:
        return
...
if __name__ == '__main__':
    """
    Isula Dissanayake (heyisula)
    Adapted from Andrew Grant's (Andrew6rant) GitHub profile README generator.
    """
    ...
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2004, 8, 15))
    ...
    print('\033[F\033[F\033[F\033[F\033[F\033[F\033[F\033[F\033[F',
        '{:<21}'.format('Total function time:'), '{:>11}'.format('%.4f' % (user_time + age_time + loc_time + commit_time + star_time + repo_time + contrib_time + calendar_time)),
